...
shinyServer(function(input, output, session) {
  ...
  # data_plot <- reactive({read_data(paste(input$run), input$seed_nb, dir_save=dir_save)})
  
  # to do : read netcdf instead of Rdata
  read_data_nc <- function(RS,dir_save){
    RS <- gsub(' SEED:','_',RS)
    data_nc <- NetCDFtoRdata(nc_name=paste("data_retro_",RS,".nc",sep=""), nc_path=dir_save)
